...
import { MessageSquare, Copy, Plus, Folder, Search, CheckCircle2 } from "lucide-react";
  ...
  const templates = [
    {
      ...
      content: "Olá! Vi que a [Nome da Empresa] tem uma ótima presença local, mas notei que vocês ainda não possuem um site próprio. Sou especialista em digitalização de negócios locais e ajudo clínicas a captarem mais. Podemos falar rapidinho hoje à tarde?",
    },
    {
      title: "Abordagem de Valor - Google Meu Negócio",
      objective: "Criar urgência",
      content: "Bom dia! Encontrei vocês no Google, mas reparei que há poucas avaliações recentes. Eu ajudo empresas do seu setor a rankearem melhor e atraírem mais clientes pela busca. Tem 5 minutos para eu mostrar como fazer isso?",
    }
  ];
  ...
  return (
    <div className="w-full max-w-[1400px] mx-auto p-4 md:p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-text-primary tracking-tight">Biblioteca de Mensagens</h1>
          <p className="text-sm text-text-muted mt-1">Sua base de conhecimento para prospecção no WhatsApp.</p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <aside className="w-full md:w-[240px] shrink-0 space-y-4">
          ...
          <div className="flex flex-col gap-1">
            <h4 className="text-[10px] font-bold text-text-muted uppercase tracking-wider mb-2 px-2">Categorias</h4>
            {tabs.map((tab) => (
              <button
                key={tab.name}
              ...
              >
                <div className="flex items-center gap-2">
                  <Folder className={`w-4 h-4 ${tab.active ? "fill-primary/20" : ""}`} />
                  {tab.name}
                </div>
                <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${
                  tab.active ? "bg-primary/20 text-primary" : "bg-surface-elevated text-text-muted border border-border-subtle"
                }`}>
                  {tab.count}
                </span>
              </button>
        ...
        <div className="flex-1 space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {templates.map((tpl, i) => (
              <DashboardSurface key={i} className="flex flex-col justify-between p-5 hover:border-border-strong transition-colors group">
                <div>
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <h3 className="font-semibold text-text-primary text-sm leading-tight">{tpl.title}</h3>
                ...
                <div className="pt-4 mt-4 border-t border-border-subtle flex items-center justify-between">
                  <Button variant="ghost" size="sm" className="h-8 text-xs text-text-secondary hover:text-text-primary -ml-2">
                    Editar
                  </Button>
            ...
            <DashboardSurface className="flex flex-col items-center justify-center p-6 border-dashed border-border-strong bg-transparent hover:bg-surface-hover/50 hover:border-primary/50 transition-colors cursor-pointer group min-h-[200px]">
              ...
              <h4 className="text-sm font-medium text-text-primary mb-1">Criar nova abordagem</h4>
