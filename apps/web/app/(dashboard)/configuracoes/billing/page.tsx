    ...
    <div className="flex flex-col gap-8 p-8 max-w-6xl mx-auto w-full">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">
          Gerencie sua assinatura, limites e créditos avulsos.
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <div className="p-6 border rounded-xl bg-card">
          ...
          {profile.subscriptionStatus && (
            <p className="text-sm text-emerald-500 mt-2 font-medium capitalize">
            ...
            <div>
              <p className="text-2xl font-bold">
                ...
                <span className="text-muted-foreground text-lg font-normal">
                  / {config.searchesPerDay}
                </span>
              </p>
              <p className="text-sm text-muted-foreground mt-2">
                Restantes:{" "}
                {Math.max(0, (config.searchesPerDay || 0) - usedToday)}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="p-6 border rounded-xl bg-card">
          <p className="text-sm font-medium text-muted-foreground mb-1">
            Fetch Credits (Avulsos)
          </p>
          <p className="text-2xl font-bold">{credits}</p>
          <p className="text-sm text-muted-foreground mt-2">
            1 crédito = 1 busca extra
          </p>
        </div>
      </div>
