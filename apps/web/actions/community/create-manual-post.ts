...
export async function createManualPost(formData: FormData) {
  try {
    ...
    if (!user) {
      return { error: "UNAUTHORIZED: Você precisa estar logado para publicar." };
    }
    ...
    const parsed = CreateManualPostSchema.safeParse({ type, content, templateMetadata });

    if (!parsed.success) {
      return { error: "INVALID_POST: Dados inválidos.", details: parsed.error.flatten() };
    }
    ...
    if (data.type === "template" && data.templateMetadata) {
      await db.transaction(async (tx) => {
        const [post] = await tx.insert(communityPosts).values({
          authorId: user.id,
          type: data.type,
