...
export function CommunityFeed({ initialData, tab }: { initialData: any, tab: "community" | "following" }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 border-b border-slate-800 pb-2">
        <Link 
          ...
          className={`text-sm font-medium pb-2 -mb-2 border-b-2 transition-colors ${
            tab === "community" ? "border-orange-500 text-white" : "border-transparent text-slate-400 hover:text-slate-200"
          }`}
        >
          Comunidade
        </Link>
        <Link 
          ...
          className={`text-sm font-medium pb-2 -mb-2 border-b-2 transition-colors ${
            tab === "following" ? "border-orange-500 text-white" : "border-transparent text-slate-400 hover:text-slate-200"
          }`}
        >
          Seguindo
        </Link>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex items-center gap-4 cursor-pointer hover:bg-slate-800/80 transition-colors">
        ...
        <div className="text-slate-400 flex-1">O que você aprendeu ou conquistou hoje?</div>
      ...
      <div className="space-y-4">
        {items.length === 0 ? (
          <div className="text-center py-12 bg-slate-900/50 rounded-2xl border border-slate-800/50">
            <h3 className="text-lg font-medium text-slate-200 mb-2">Nenhuma pista farejada ainda...</h3>
            <p className="text-slate-400 text-sm max-w-sm mx-auto">
              O Cyber-Caramelo ainda não encontrou movimentação por aqui. Compartilhe uma estratégia, resultado ou dúvida com a Matilha.
            </p>
          </div>
        ) : (
          items.map((post: any) => (
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-slate-200">{post.author.displayName}</span>
                    <span className="text-xs bg-slate-800 text-slate-400 px-2 py-0.5 rounded-full">Nv. {post.author.level}</span>
                  </div>
                ...
                <div className="bg-slate-950 rounded-xl p-4 border border-slate-800/60 mb-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div>
                      ...
                      <div className="text-purple-400 font-medium">{post.metadata.niche}</div>
                    </div>
                    {post.metadata.leadScore && (
                      <div>
                        <div className="text-slate-500 mb-1">Lead Score</div>
                        <div className="text-slate-200">{post.metadata.leadScore}/100</div>
                      </div>
                    )}
                    <div>
                      <div className="text-slate-500 mb-1">Fechamento</div>
                      <div className="text-slate-200">{post.metadata.closeDays} dias</div>
                    </div>
                    {post.metadata.channel && (
                      <div>
                        <div className="text-slate-500 mb-1">Canal</div>
                        <div className="text-slate-200 capitalize">{post.metadata.channel}</div>
                      </div>
                    )}
                  </div>
                </div>
