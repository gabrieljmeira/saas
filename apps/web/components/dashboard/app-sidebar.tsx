  ...
  return (
    <aside className="hidden md:flex w-[240px] shrink-0 flex-col bg-surface border-r border-border-default h-screen sticky top-0">
          ...
          <div key={i} className="flex flex-col gap-1.5">
            ...
            {group.items.map((item) => {
              ...
              return (
                ...
                >
                  <item.icon className={`w-4 h-4 transition-colors ${isActive ? "text-primary" : "text-text-muted group-hover:text-text-primary"}`} />
                  {item.name}
                </Link>
              );
            })}
      ...
      <div className="p-3 border-t border-border-subtle shrink-0">
        ...
        >
          <Settings className={`w-4 h-4 transition-colors ${pathname.startsWith("/configuracoes") ? "text-primary" : "text-text-muted group-hover:text-text-primary"}`} />
          Configurações
        </Link>
      </div>
    </aside>
