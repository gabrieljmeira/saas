...
export function LeadDrawer({ leadId, open, onOpenChange }: LeadDrawerProps) {
  ...
  const whatsappUrl = normalizedPhone
    ? generateWhatsappUrl(normalizedPhone, approachMessage)
    : null;
        ...
        {loading || !lead ? (
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="p-6 md:p-8 bg-surface border-b border-border-default">
                ...
                <div className="text-right">
                  ...
                  <Badge
                    ...
                    className={`text-lg px-3 py-1 ${
                      (lead.leadScore as number) >= 70
                        ? "border-primary/30 text-primary bg-primary/5"
                        : (lead.leadScore as number) >= 40
              ...
              <div className="flex flex-wrap gap-2 mt-6">
                {lead.website ? (
                  <Badge
                    variant="secondary"
                    className="bg-surface-elevated font-normal gap-1 text-text-secondary"
                  >
                  ...
                  <Badge
                    variant="secondary"
                    className="bg-destructive/10 text-destructive border-transparent font-normal gap-1"
                  >
                ...
                {hasWhatsapp && (
                  <Badge
                    variant="secondary"
                    className="bg-green-500/10 text-green-500 border-transparent font-normal gap-1"
                  >
                ...
                {Boolean(lead.instagram) && (
                  <Badge
                    variant="secondary"
                    className="bg-pink-500/10 text-pink-500 border-transparent font-normal gap-1"
                  >
                    ...
                    <ul className="space-y-2">
                      {leadScoreReasons.map((r: unknown, i: number) => {
                        ...
                        return (
                          <li
                            key={i}
                            className="flex items-start justify-between text-sm text-text-muted bg-surface p-2 rounded border border-border-subtle"
                          >
                ...
                {isPending ? (
                  <div className="bg-surface-elevated border border-border-default rounded-lg p-8 flex flex-col items-center justify-center text-center animate-in fade-in">
                    ...
                    <p className="text-xs text-text-muted mt-1">
                      Gerando abordagem personalizada baseada no score...
                    </p>
                  </div>
                ) : approachMessage ? (
                  ...
                  <div className="text-sm text-text-muted p-6 bg-surface rounded-lg border border-border-dashed border-border-strong text-center">
                    Gere uma mensagem personalizada para iniciar o contato de
                    forma assertiva.
                  </div>
                )}
              </div>
            ...
            <div className="p-6 bg-surface border-t border-border-default flex gap-3">
              ...
              <Button className="flex-1">Adicionar ao Pipeline</Button>
            </div>
          </div>
        )}
      </SheetContent>
